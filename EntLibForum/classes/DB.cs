...
namespace yaf
{
	...
	public class DB
	{
        ...
        /// <summary>
        ///  更新帖子 TOPIC 的 HasAttachments 字段
        /// </summary>
        /// <param name="messageID"></param>
        static public void attachment_updatetopic(object messageID)
        {
            string strUpdate = @"Update yaf_topic set HasAttachments=1 
                    from yaf_topic yt with (NOLOCK) inner join yaf_message ym with (NOLOCK) on yt.topicid=ym.topicid
                    where ym.messageid=" + messageID.ToString() +" and yt.HasAttachments<>1";

            SqlHelper.ExecuteNonQuery(strUpdate);
        }
        ...
        static public DataTable forum_getspecifiedforum(object forumID)
        {
            string strSelect = "Select ForumID, ForumIcon from yaf_Forum with (nolock) where forumID=" + forumID.ToString();

            return GetData(strSelect);
        }

        static public void forum_updatelogo(object forumID, object forumLogo)
        {
            string strUpdate = "Update yaf_forum set ForumIcon='" + forumLogo.ToString() + "' where forumID=" + forumID.ToString();

            SqlCommand sqlCommand = new SqlCommand(strUpdate);
        ...
        static public void topic_setdigest(object topicID, int digestLevel)
        {
            string strUpdate = "Update yaf_Topic set DigestLevel=" + digestLevel.ToString() + " Where topicID=" + topicID.ToString();
        ...
        static public void topic_setrecommend(object topicID, int isRecommend)
        {
            string strUpdate = "Update yaf_Topic set ApproveStatus=" + isRecommend.ToString() + " Where topicID=" + topicID.ToString();
				...
				using ( SqlTransaction trans = conn.BeginTransaction( m_isoLevel ) )
				{
					try
					{
						using ( SqlCommand cmd = new SqlCommand( "yaf_user_save", conn ) )
						{
                            ...
                            /// 2008-05-07
                            /// Password 不加密
							cmd.Parameters.AddWithValue( "@Password", password); // FormsAuthentication.HashPasswordForStoringInConfigFile( password.ToString(), "md5" ) );
                            //////////////////////////////////////////////////////////
							cmd.Parameters.AddWithValue( "@Email", email );
							cmd.Parameters.AddWithValue( "@Hash", hash );
							cmd.Parameters.AddWithValue( "@Location", location );
