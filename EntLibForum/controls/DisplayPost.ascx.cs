namespace yaf.controls
{
	...
	public partial class DisplayPost : BaseUserControl
	{
		...
		{
			if ( IsSponserMessage ) return "<img src='/images/common/entlib120_ad.gif' width=120 height=120 />";
        ...
        protected string GetRowNumber()
        {
            int rowNumber = Convert.ToInt32(DataRow["RowNumber"]);
            int userID = Convert.ToInt32(DataRow["UserID"]);

            if (rowNumber == 1 && userID > 1)
                return "楼 主";
            else if (rowNumber == 1 && userID == 1)
                return "AD";
            else
                return DataRow["RowNumber"].ToString() + " 楼";
        }
		...
		protected string FormatBody()
		{
            ...
            if (Convert.ToInt32(DataRow["RowNumber"]) == 1 
                && Convert.ToInt32(DataRow["UserID"]) > 1
                && ForumPage.BoardSettings.FirstPostAd.Trim().Length > 0)
            {
                messageOutput.Append(ForumPage.BoardSettings.FirstPostAd);
            }
        ...
        protected string SignatureContent()
        {
            System.Text.StringBuilder messageOutput = new System.Text.StringBuilder( 2000 );
            ...
            if (ForumPage.BoardSettings.AllowSignatures && DataRow["Signature"] != DBNull.Value && DataRow["Signature"].ToString().ToLower() != "<p>&nbsp;</p>" && DataRow["Signature"].ToString().Trim().Length > 0)
            {
                ...
                messageOutput.Append("<br/><div class='HrLine'></div>" + FormatMsg.FormatMessage(ForumPage, DataRow["Signature"].ToString(), tFlags));
            }
		...
		private void AddAttachedFiles( ref System.Text.StringBuilder messageOutput )
		{
			...
			if ( long.Parse( DataRow ["HasAttachments"].ToString() ) > 0  
                /* && base.ForumPage.User.IsAuthenticated */)
			{
						...
						foreach ( DataRow dr in dt.Rows )
						{
							...
							else if ( !bShowImage && tmpDisplaySort == 0 )
							{
								if ( bFirstItem )
								{
                                    ...
                                    messageOutput.Append(@"<img src=""images/topics/attachment2.gif"" alt=""附件"" /><br />");
									bFirstItem = false;
								}
            ...
            #region New Function - 需要登录后才允许下载
            /*
            else if (long.Parse(DataRow["HasAttachments"].ToString()) > 0)
            {
                messageOutput.Append("<p>");

                messageOutput.AppendFormat(@"<b class=""smallfont"">{0}</b><br />", ForumPage.GetText("ATTACHMENTS"));
                string strFileIcon = ForumPage.GetThemeContents("ICONS", "ATTACHED_FILE");
                messageOutput.AppendFormat(@"<img border=""0"" alt="""" src=""{0}"" /> ", strFileIcon);

                messageOutput.AppendLine("<span style='color: blue;'>需要登录后才允许下载！</span><br />");
            }*/
            #endregion
        }
		...
		private void Delete_Click( object sender, EventArgs e )
		{
			...
			DB.user_removepoints( tmpUserID, 10 );
