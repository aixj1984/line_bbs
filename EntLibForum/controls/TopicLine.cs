...
namespace yaf.controls
{
	...
	public class TopicLine : BaseControl
	{
		...
		protected override void Render( System.Web.UI.HtmlTextWriter writer )
		{
            ...
            string topic_url = "";
            if (FindUnread)
                topic_url = Forum.GetLink(Pages.posts, "t={0}&find=unread", m_row["LinkTopicID"]);
            else
                topic_url = Forum.GetLink(Pages.posts, "t={0}", m_row["LinkTopicID"]);

			// Icon
			string imgTitle = "", img = GetTopicImage( m_row, ref imgTitle );
            html.AppendFormat(@"<td><a href=""{0}"" title=""{2}"" target=""_blank""><img src=""{1}""></a></td>",topic_url,img, imgTitle);
            ...
            // 显示 精华贴 的 标识
            ////////////////////////////////////////////////////////////////////////////////////
            if (m_row["HasAttachments"] != System.DBNull.Value && Convert.ToInt32(m_row["HasAttachments"]) > 0)
            {
                html.Append("&nbsp;<img src='images/topics/attachment.gif' alt='附件' align='absbottom' />");
            }
			...
			html.Append( "</td>" );
			// Topic Starter
            html.AppendFormat( @"<td align=""left"" class=""smallfont"">{0}</td>", FormatStartInfo());
			...
			// Last Post
			html.AppendFormat( @"<td align=""left"" class=""smallfont"">{0}</td>", FormatLastPost() );
        ...
        protected string FormatStartInfo()
        {
            DataRowView row = m_row;
            string strReturn = "";
            if (row["UserID"].ToString().Length > 0)
            {
                string first_line = string.Format(@"<a title="""" href="""" >{0}</a>",
                        ForumPage.FormatDateTimeTopic(Convert.ToDateTime(row["Posted"])) );
                string second_line = string.Format("<a href=\"{0}\">{1}</a>",
                        Forum.GetLink(Pages.profile, "u={0}", m_row["UserID"]),
                        BBCode.EncodeHTML(m_row["Starter"].ToString()));

                strReturn = String.Format("{0}<br />{1}", second_line, first_line);
            }
		...
		protected string GetPriorityMessage( DataRowView row )
		{
			...
			else if ( row ["PollID"].ToString() != "" )
			{
				...
				{
					case 1: 
                        //strReturn = ForumPage.GetText( "STICKY" ); 
                        strReturn = "<img src=\"images/topics/sticky.gif\" alt=\"置顶\" style=\"vertical-align:text-bottom;\" />";
                        break;
					case 2: 
                        //strReturn = ForumPage.GetText( "ANNOUNCEMENT" ); 
                        strReturn = "<img src=\"images/topics/announcement.gif\" alt=\"公告\" style=\"vertical-align:text-bottom;\" />";
                        break;
				}
		...
		protected string FormatLastPost()
		{
			...
			if ( row ["LastMessageID"].ToString().Length > 0 )
			{
                string first_line = string.Format(@"<a title="""" href=""{1}"">{0}</a>", 
                        ForumPage.FormatDateTimeTopic(Convert.ToDateTime(row["LastPosted"])),
                        Forum.GetLink(Pages.posts, "m={0}#{0}", row["LastMessageID"]));
                string second_line = string.Format("<a href=\"{0}\">{1}</a>",
                        Forum.GetLink( Pages.profile, "u={0}", row ["LastUserID"] ),
		...
		protected string CreatePostPager( int Count, int PageSize, int TopicID )
		{
			...
			if ( PageCount > 1 )
			{
                ...
                // 
                strReturn = "<img src='/images/common/arrow2.gif' alt='' />&nbsp;";
                //////////////////////////////////////////////////////////////////
