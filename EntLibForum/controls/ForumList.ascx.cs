	...
	public partial class ForumList : BaseUserControl
	{
		...
		public string GetForumLink( System.Data.DataRow row )
		{
			...
			else
			{
				...
				strReturn = String.Format( "{0} <br /> {1}", strReturn, ForumPage.GetText( "NO_FORUM_ACCESS" ) );
			}
			...
			if ( !row.IsNull( "LastPosted" ) )
			{
				...
				if ( int.Parse( row ["ReadAccess"].ToString() ) > 0 )
				{
					strTemp = String.Format( "{0}<br/>{1}<br/>{2}&nbsp;<a title=\"{4}\" href=\"{5}\"><img src=\"{3}\"></a>",
							...
							String.Format( ForumPage.GetText( "in" ), String.Format( "<a href=\"{0}\">{1}</a>", Forum.GetLink( Pages.posts, "t={0}", row ["LastTopicID"] ), Truncate( Utils.BadWordReplace( row ["LastTopicName"].ToString() ), 40 ) ) ),
				...
				else
				{
					...
					strTemp = String.Format( "{0}<br/>{1}<br/>{2}",
							ForumPage.FormatDateTimeTopic( ( DateTime ) row ["LastPosted"] ),
							String.Format( ForumPage.GetText( "in" ), String.Format( "{0}", Truncate( row ["LastTopicName"].ToString(), 40 ) ) ),
        ...
        protected string GetForumLogo( object _o)
        {
        ...
        protected string GetForumLogoLink(object _o)
        {
            ...
            if (iconPath != System.DBNull.Value && iconPath.ToString().Trim().Length > 0)
            {
                strPath = "/images/forumicons/" + iconPath.ToString();
            }
