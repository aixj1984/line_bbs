...
namespace yaf.pages
{
	...
	public class ForumPage : System.Web.UI.UserControl
	{
		...
		private void ForumPage_PreRender( object sender, EventArgs e )
		{
			...
			if ( BoardSettings.AllowThemedLogo)
			{
			...
			header.AppendFormat( "<table width=100% cellspacing=0 cellpadding=0 class='mainmenu'><tr>" );
			...
			if ( User != null && User.IsAuthenticated )
			{
				...
				*/
                ...
                // =========================
				if ( User.CanLogin )
					header.AppendFormat( String.Format( "<a href=\"{0}\" onclick=\"return confirm('您确定要退出登录吗?');\">{1}</a>", Forum.GetLink( Pages.logout ), GetText( "TOOLBAR", "LOGOUT" ) ) );
			}
			else
			{
		...
		protected override void Render( System.Web.UI.HtmlTextWriter writer )
		{
			if ( m_bShowToolBar )
			{
                ...
                footer.AppendFormat("<div class='powerby'>");

				if ( BoardSettings.ShowRSSLink )
				{
					footer.AppendFormat( "{2} : <a href=\"{0}\"><img valign=\"absbottom\" src=\"{1}images/rss.gif\" alt=\"RSS\" /></a>&nbsp;&nbsp;", Forum.GetLink( Pages.rsstopic, "pg=forum" ), Data.ForumRoot, GetText( "DEFAULT", "MAIN_FORUM_RSS" ) );
